...
from mmdet.core.bbox.transforms_rotate import  poly_to_rotated_box,obb2poly_oc,rotated_box_to_poly_np
...
class FCETargets(BaseTextDetTargets):
    ...
    def __init__(self,
        ...
        self.downasmple_ratio = downasmple_ratio
    ...
    def cal_fourier_signature(self, polygon, fourier_degree):
        ...
        resampled_polygon = cv2.resize(np.array(polygon,dtype=np.float32),(2,256))#归一化点的数量
    ...
    def generate_fourier_maps(self, img_size, text_polys,gt_labels):
        ...
        h, w = img_size
        text_region_mask = np.zeros((int(h/self.downasmple_ratio), int(w/self.downasmple_ratio)), dtype=np.uint8)
        k = self.fourier_degree
        fourier_coeff=[]

        gt_rboxes, gt_bboxes,gt_resampled_polygon=[], [],[]
        for ind, poly in enumerate( text_polys):
            #处理一个图形有多个轮廓的问题，就是取面积最大的那个轮廓
            if  len(poly) >1:
                areas=[]
                for pol in poly:
                    # text_instance = [[pol[i], pol[i + 1]]
            ...
            # gt_rboxes.append(poly2obb_np_mask(poly))
            gt_rboxes.append(poly_to_rotated_box(poly,gt_labels[ind], self.circular_class))
            # x, y, b_w, b_h, a = poly2obb_np_mask(poly)
            ...
            polygon = np.array(text_instance,dtype=np.int).reshape((1, -1, 2))
            class_index =1# np.int(gt_labels[ind]+1)# 这里要从1开始，如果是0无法和背景区分
            cv2.fillPoly(text_region_mask, np.array(polygon/self.downasmple_ratio,dtype=np.int), class_index)
            
            resampled_polygon = cv2.resize(np.array(polygon[0],dtype=np.float32),(2,self.num_sample))#归一化点的数量
            resampled_polygon =  self.normalize_polygon(resampled_polygon).reshape(-1)
            gt_resampled_polygon.append(resampled_polygon)
            
            fourier_coeff.append( self.cal_fourier_signature(polygon[0], k))
        return np.array(fourier_coeff), text_region_mask, np.array(gt_rboxes), np.array(gt_bboxes), np.array(gt_resampled_polygon)
    ...
    def generate_targets(self, results):
        ...
        level_img_size = (h , w )
        fourier_coeff , text_region, gt_rboxes, gt_bboxes,gt_resampled_polygon= self.generate_fourier_maps(
            level_img_size, lv_text_polys,gt_labels)
        # try:
        if self.output_points:
            gt_masks = gt_resampled_polygon
        else:
            fourier_coeff = fourier_coeff.reshape(fourier_coeff.shape[0],-1)
            gt_masks = np.concatenate((gt_rboxes,fourier_coeff) ,axis = 1)
        # except:
        #     print(gt_rboxes)
