...
def get_bbox_from_polygon(pred, anchor_pts):
    num_poly_pts = int(pred[:,:, ...].size(1)/4)
    pred_reshape = pred[:,:,...].view(pred.shape[0], -1, 2, *pred.shape[2:])
    pred_pts, inds = torch.max(pred_reshape, dim=2)
    neg_inds = inds == 0
    pred_pts[neg_inds] *= -1
    pred_pts = pred_pts.view(pred_pts.shape[0], -1, 2)
...
def cross_iou_loss(pred, target, loss_type=None, anchor_pts=None, vs=None,
                   bbox_gt=None, bbox_pred=None, eps=1e-6, alpha=0.2, stride=9):
...
class CrossIOULoss(nn.Module):
    ...
    def __init__(self, eps=1e-6, reduction='mean', loss_weight=1.0, loss_type='bbox', alpha=0.2, stride=12):
