import torch
from mmdet.core.bbox.transforms_rotate import rotated_box_to_poly,ploys2rboxes
from mmcv.ops import nms_rotated

def multiclass_nms_rotated_2box(multi_ploys,
                           multi_scores,
                           score_thr,
                           nms,
                           max_num=-1,
    ...
    multi_bboxes = ploys2rboxes( multi_ploys)
    ...
    if multi_bboxes.shape[1] > 5:
        ...
        ploys =  multi_ploys[:, None].expand(
            multi_scores.size(0), num_classes, multi_ploys.shape[-2],multi_ploys.shape[-1])
    ...
    ploys = ploys.reshape(-1,multi_ploys.shape[-2],multi_ploys.shape[-1])
    ...
    a = ploys
    r2bboxes = torch.stack([a[:, :, 0].min(1)[0],a[:, :,1].min(1)[0],a[:, :,0].max(1)[0],a[:, :,1].max(1)[0]],-1)
    ...
    if return_inds:
        return  torch.cat([r2bboxes, scores[:, None]], 1), labels,keep
    else:
        return    torch.cat([r2bboxes, scores[:, None] ], 1), labels,ploys


# def multiclass_nms_rotated_2box(multi_bboxes,
#                            multi_scores,
#                            score_thr,
#                            nms,
#                            max_num=-1,
...
#     if return_inds:
#         return  torch.cat([r2bboxes, scores[:, None], bboxes, points], 1), labels,keep
#     else:
#         return    torch.cat([r2bboxes, scores[:, None], bboxes, points], 1), labels
#     # torch.cat([bboxes, scores[:, None]], 1), labels
